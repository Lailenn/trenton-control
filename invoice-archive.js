...
window.InvoiceArchive = function (app) {
  ...
  async function deleteRecord(record) {
    if (!confirm(`¿Eliminar la invoice ${record.invoiceNumber || ""} de ${record.address || "esta dirección"}? Saldrá de la web y de la nube.`)) return;
    try {
      await app.remove(record);
      await app.reload();
      app.render();
      app.toast("Invoice eliminada.");
    } catch (error) {
      app.toast(error.message || "No se pudo eliminar la invoice.");
    }
  }
